...
14. 例题[HDU-1024](https://vjudge.net/problem/HDU-1024)，AC[源码](./HDU-1024.cc)
    ...
    这题我做了几天。网上的题解大多对状态的定义描述的不够清楚。此题也可以这样定义状态：dp\[i][j]表示前i个元素构成j组，且以第i个元素结尾的最大和。状态的转移为：dp\[i][j] = std::max(dp\[i-1][j] + A[i], dp\[k][j-1] + A[i] 1<=k<=i-1)。因为以第i个元素结尾，所以这样定义的状态一定要选第i个元素。显然第i个元素要么加入前面的一组(即dp\[i-1][j])，要么自己单独构成一组。

    此外这题必须要滚动数组，不然会超内存。滚动数组的写法和前面的例题(如**例题8**)是一样的，不过需要注意，滚动几次后，滚动数组需要重新初始化。
